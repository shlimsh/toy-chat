...
exports.SAFE_ATTRIBUTES = exports.CHARACTER_LIMIT = void 0;
exports.getComposedPathSelector = getComposedPathSelector;
 ...
 */
exports.CHARACTER_LIMIT = 2 * browser_core_1.ONE_KIBI_BYTE;
/**
 ...
 */
exports.SAFE_ATTRIBUTES = getSelectorFromElement_1.STABLE_ATTRIBUTES.concat([
    'role',
    'type',
    'disabled',
    'readonly',
...
function getComposedPathSelector(composedPath, actionNameAttribute) {
    ...
    const allowedAttributes = actionNameAttribute ? [actionNameAttribute].concat(exports.SAFE_ATTRIBUTES) : exports.SAFE_ATTRIBUTES;
    let result = '';
    for (const element of elements) {
        const part = getSelectorStringFromElement(element, allowedAttributes);
        result += part;
        if (result.length >= exports.CHARACTER_LIMIT) {
            return (0, browser_core_1.safeTruncate)(result, exports.CHARACTER_LIMIT);
        }
    }
    return result;
}
...
function getSelectorStringFromElement(element, allowedAttributes) {
    const tagName = (0, getSelectorFromElement_1.getTagNameSelector)(element);
    const id = (0, getSelectorFromElement_1.getIDSelector)(element);
...
function getElementClassesString(element) {
    return Array.from(element.classList)
        .filter((c) => !(0, getSelectorFromElement_1.isGeneratedValue)(c))
        .sort()
        .map((c) => `.${CSS.escape(c)}`)
        .join('');
}
...
function computePositionDataString(element) {
    ...
    const nthOfType = (0, getSelectorFromElement_1.getNthOfTypeSelector)(element);
...
function extractSafeAttributesString(element, allowedAttributes) {
    ...
    for (const attribute of attributes) {
        if (allowedAttributes.includes(attribute.name)) {
            result.push((0, getSelectorFromElement_1.getAttributeValueSelector)(attribute.name, attribute.value));
        }
    }
    return result.sort().join('');
}
//# sourceMappingURL=getComposedPathSelector.js.map